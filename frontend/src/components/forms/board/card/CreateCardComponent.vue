<template>
    <h5 class="text-center mt-2">Create new card</h5>
    <hr class="mb-4" />
    <div class="w-100 d-flex flex-column flex-lg-row justify-content-center 
        align-items-center gap-3">
        <form class="form-create-card w-100 bg-white p-3" @submit.prevent="handleForm" enctype="multipart/form-data">
            <div class="d-flex flex-column justify-content-center align-items-center mb-4">
                ...
                <input type="text" class="form-control border border-dark border-1 text-center mb-2" id="title"
                    placeholder="Enter the title" v-model="storeForm.formData.createCard.title"
                    @input="storeForm.validateTitle('createCard')">
                <p class="w-100 text-center text-danger mt-2 fw-bold">
                    {{ storeForm.formErrors.createCard.title }}
                </p>
            </div>
            <div class="d-flex flex-column justify-content-center align-items-center mb-4">
                ...
                <input type="text" class="form-control border border-dark border-1 text-center mb-2" id="description"
                    placeholder="Enter the description" v-model="storeForm.formData.createCard.description">
            </div>
            <p class="w-100 text-center showErrorForm fw-bold mb-5 text-danger">
                {{ storeForm.formErrors.createCard.form }}
            </p>
            <ResponseApiComponent :loading="loading" :entity="storeApp.getEntity(entity)">
                <template #response-api>
                    <p :class="storeApp.getResponseApi('card.create').class" class="w-100 text-center fw-bold">
                        {{ storeApp.getResponseApi('card.create').message }}
                    </p>
                </template>
            ...
            <div class="modal-container-buttons d-flex flex-row justify-content-center
                ...
                <button type="submit" class="btn btn-primary w-75 d-block m-auto btnCreateCard">
                    Create
                </button>
            </div>
        </form>
...
const handleForm = async (e) => {
    ...
    const formHandlers = {
        "form-create-card": {
            fields: ["title"],
            action: () => storeBoard.createCard(storeForm.fillFormData("createCard")),
            btnName: "btnCreateCard",
            keyForm: "createCard",
            entity: "card.create",
            errorMessages: () => storeForm.errorValidations.createCard.form.requiredFields,
            optionsRedirection: {
